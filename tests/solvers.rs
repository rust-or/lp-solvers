...
fn cbc_optimal() {
    ...
    } = solver.read_solution::<Problem>(&"cbc_optimal.sol".to_string(), None).unwrap();
...
fn cbc_infeasible() {
    ...
    let Solution { status, .. } = solver.read_solution::<Problem>(&"cbc_infeasible.sol".to_string(), None).unwrap();
...
fn cbc_unbounded() {
    ...
    let Solution { status, .. } = solver.read_solution::<Problem>(&"cbc_unbounded.sol".to_string(), None).unwrap();
...
fn glpk_optimal() {
    ...
    let Solution { status, results: mut variables, .. } = solver.read_solution::<Problem>(&"glpk_optimal.sol".to_string(), None).unwrap();
    assert_eq!(status, Status::Optimal);
...
fn glpk_infeasible() {
    ...
    let Solution { status, .. } = solver.read_solution::<Problem>(&"glpk_infeasible.sol".to_string(), None).unwrap();
    assert_eq!(status, Status::Infeasible);
}
...
fn glpk_unbounded() {
    ...
    let Solution { status, .. } = solver.read_solution::<Problem>(&"glpk_unbounded.sol".to_string(), None).unwrap();
    assert_eq!(status, Status::Unbounded);
}
...
fn glpk_empty_col_bounds() {
    ...
    let Solution { status, results: solution, .. } = solver.read_solution::<Problem>(&"glpk_empty_col_bounds.sol".to_string(), None).unwrap();
    assert_eq!(status, Status::Optimal);
